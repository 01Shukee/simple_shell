...
ptree_t *parse_expr(unsigned int *ntoken, tokens_t *tokens, ptree_t *lhs, int min_prec)
{
	...
	while (*ntoken < tokens->tokensN && tokens->tokens[lookahead].prec >= min_prec)
	{
		...
		*ntoken = *ntoken + 1;
		...
		{
			lookahead = *ntoken;
			while (*ntoken < tokens->tokensN && tokens->tokens[lookahead].id != TOKEN_STRING &&
				   tokens->tokens[lookahead].prec > tokens->tokens[op].prec)
			{
...
/**
 * parse - Main function that creates the entire parse tree from the tokenizer.
